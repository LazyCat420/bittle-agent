...
- Before every patch after the first, call `bittle_diagnose_run` on the last run and read it IN THIS ORDER: (1) `train_vs_bench` + `terrain_gap` -- if the training curve walks >1.5x the benchmark and the terrain is easier than the protocol, the optimiser solved an easier field than the exam: raise `terrain.box_height_m` / `terrain.n_boxes` / `terrain.slope_deg` to cover the protocol FIRST (terrain keys are a first-class lever, not a last resort); (2) `reward_terms_off` -- a gate whose term has weight 0.0 cannot be learned, set the weight; (3) a term below ~1% share cannot steer the policy, change its weight by 10-100x (bounds allow up to |10|), not by 2x; (4) a gate that did not move vs the parent while its term already carries >=2% is a BUDGET problem, not a weight problem. `stuck` tells you where and whether the legs catch edges.
- Change at most 3 config keys per cycle and put your hypothesis in `notes`. Budget: `ppo.num_timesteps` 10000000 (10M, ~4 min) for a reward-weight tweak on a warm start; 30000000 (30M, ~12 min) when the terrain changes or a gait has to be reshaped (foot clearance, stumble, turning); 40M from scratch only when the network or the observation changes.
