...
async def training_rollout(run_id: str, seed: str = "best", suite: str | None = None):
    if not agent_harness.trainer.configured:
        raise HTTPException(status_code=503, detail="trainer_not_configured")
    return _proxy_stream(f"/runs/{run_id}/rollout?seed={seed}" + (f"&suite={suite}" if suite else ""))


@app.get("/api/training/artifact/{run_id}/{name:path}")
async def training_artifact(run_id: str, name: str):
    """A file from the run directory on the trainer (benchmark/<suite>@<v>/best.gif, rollouts, logs)."""
    if not agent_harness.trainer.configured:
        raise HTTPException(status_code=503, detail="trainer_not_configured")
    media = "image/gif" if name.endswith(".gif") else ("application/json" if name.endswith(".json") else "application/octet-stream")

    async def gen():
        async for chunk in agent_harness.trainer.rollout_stream(f"/runs/{run_id}/artifacts/{name}"):
            yield chunk

    return StreamingResponse(gen(), media_type=media, headers={"Cache-Control": "public, max-age=3600"})
...
# ── Documentation (generated by documentation/build_docs.py) ──────────────
DOCS_DIR = Path(__file__).resolve().parent.parent / "documentation"


@app.get("/documentation", include_in_schema=False)
async def documentation_redirect():
    # Trailing slash so the page's relative media links resolve under /documentation/.
    return RedirectResponse(url="/documentation/", status_code=307)


@app.get("/documentation/", include_in_schema=False)
async def documentation_page():
    """Serve the repo's working documentation on the server that already runs.

    Markdown chapters are the source of truth; this serves the page that
    ``python3 documentation/build_docs.py`` generates (and ``--check`` keeps
    honest). The media folder (training GIFs, plots) is mounted beside it.
    """
    index = DOCS_DIR / "index.html"
    if not index.is_file():
        raise HTTPException(status_code=503, detail="documentation/index.html is not built: run python3 documentation/build_docs.py")
    return FileResponse(index, media_type="text/html")


if (DOCS_DIR / "media").is_dir():
    app.mount("/documentation/media", StaticFiles(directory=str(DOCS_DIR / "media")), name="doc-media")
